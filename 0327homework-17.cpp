...
int checkDeno(int d1, int d2) { // 檢查分母，並返回分數的最簡形式的分母
    if (d1 == 0 || d2 == 0) {
        throw "divided by zero";
    }
...
Fraction operator+(const Fraction &f1, const Fraction &f2) {
    ...
    int deno = checkDeno(d1, d2);  // 計算兩分母的最小公倍或最大公因數，成為新分數的分母
...
bool operator==(const Fraction &f1, const Fraction &f2) {
    ...
    if (sim_n1 == sim_n2 && sim_d1 == sim_d2) { //// 如果兩個分數的最簡形式相同，則返回 true
        return true;
    }
