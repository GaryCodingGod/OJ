...
public:
    ...
    Fraction change(int num, int deno) { //把分母負號轉到分子
    ...
    Fraction operator*(const Fraction& f) {
        int n1 = this->getNumerator(); //使用this指標指向資料成員(物件)
        int n2 = f.getNumerator();
        int d1 = this->getDenominator();
        int d2 = f.getDenominator();
        int num = n1 * n2;    
    ...
    friend ostream& operator<<(ostream &os, Fraction f) {
        ...
        f = f.change(f.numerator / gcd, f.denominator / gcd);
    ...
    void display() {
        ...
        Fraction temp = change(this->numerator / gcd, this->denominator / gcd);
